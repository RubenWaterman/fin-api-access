...
defmodule FinAPIAccess.Api.Authorization do
  @moduledoc """
  ...
  - grant_type (String.t): Determines the required type of authorization:password - authorize a user; client_credentials - authorize a client;refresh_token - refresh a user's access_token.
  - client_id (String.t): Client identifier
  - client_secret (String.t): Client secret
  - opts (KeywordList): [optional] Optional parameters
    - :refresh_token (String.t): Refresh token. Required for grant_type=refresh_token only.
    - :username (String.t): User identifier. Required for grant_type=password only.
    - :password (String.t): User password. Required for grant_type=password only.
  ## Returns

  {:ok, %FinAPIAccess.Model.AccessToken{}} on success
  {:error, info} on failure
  """
  @spec get_token(Tesla.Env.client, String.t, String.t, String.t, keyword()) :: {:ok, FinAPIAccess.Model.ErrorMessage.t} | {:ok, FinAPIAccess.Model.AccessToken.t} | {:ok, FinAPIAccess.Model.BadCredentialsError.t} | {:error, Tesla.Env.t}
  def get_token(connection, grant_type, client_id, client_secret, opts \\ []) do
    optional_params = %{
      :"refresh_token" => :form,
      :"username" => :form,
      :"password" => :form
    ...
    |> evaluate_response([
      { 200, %FinAPIAccess.Model.AccessToken{}},
  ...
  - connection (FinAPIAccess.Connection): Connection to server
  - token (String.t): The token that the client wants to get revoked
  - opts (KeywordList): [optional] Optional parameters
    - :token_type_hint (String.t): A hint about the type of the token submitted for revocation
  ## Returns

  {:ok, %{}} on success
  {:error, info} on failure
  """
  @spec revoke_token(Tesla.Env.client, String.t, keyword()) :: {:ok, nil} | {:ok, FinAPIAccess.Model.ErrorMessage.t} | {:error, Tesla.Env.t}
  def revoke_token(connection, token, opts \\ []) do
    optional_params = %{
      :"token_type_hint" => :query
    }
    %{}
    |> method(:post)
    |> url("/oauth/revoke")
